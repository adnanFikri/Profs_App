...
<div class="h-screen md:flex">
        <div style="background-image: url(img/bg-login.jpg);background-repeat: no-repeat;background-size: cover;"
        ...
        <div class="flex md:w-1/2 justify-center py-10 items-center bg-white">
            <form class="bg-white" method="POST" action="{{ route('register') }}">
                @csrf
                <h1 class="text-gray-800 font-bold text-2xl mb-3 text-center">REGISTER</h1>
                ...
                @guest
                    @if (Route::has('login'))
                    <a href="{{ route('login') }}" span class="text-sm ml-2 hover:text-blue-500 cursor-pointer">Vous avez de
                        compte
                        ?</span></a>
                    @endif
                @endguest
